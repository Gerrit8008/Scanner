...
def get_severity_level(severity):
    """Return numeric severity level for sorting and prioritization"""
...
def get_recommendations(vulnerability, severity):
    """Generate standardized recommendations based on severity level"""
...
def generate_threat_scenario(vulnerability, severity):
    """Generate specific threat scenarios for different vulnerability types"""
    scenarios = {
...
def check_os_updates():
    """Check for operating system updates and security patches"""
    try:
...
def get_windows_version():
    """Check Windows version and assess security risk based on version"""
    try:
        os_name = platform.system()
        if os_name == "Windows":
            try:
...
def check_dmarc_record(domain):
    ...
    try:
        ...
        for rdata in answers:
            ...
            if record.lower().startswith("v=dmarc1"):
                ...
                if policy_match:
                    ...
                    if policy == "none":
                        return f"DMARC record found with monitoring policy (p=none): {record}", "Medium"
                    elif policy == "quarantine":
                        return f"DMARC record found with quarantine policy: {record}", "Low"
                    elif policy == "reject":
                        return f"DMARC record found with reject policy: {record}", "Low"
                    else:
                        return f"DMARC record found with unknown policy: {record}", "Medium"
                return f"DMARC record found but missing policy: {record}", "Medium"
        ...
        logging.error(f"Error checking DMARC record: {e}")
...
def check_dkim_record(domain):
    ...
    for selector in selectors:
        ...
        try:
            ...
            logging.error(f"Error checking DKIM with selector {selector}: {e}")
            continue  # Try next selector

    return "DKIM record not found using common selectors.", "High"

def analyze_email_security(domain):
    """Combine all email security checks into one comprehensive analysis"""
    try:
        result = {
            'domain': domain,
            'spf': {},
        ...
        if dmarc_severity != 'Low':
            result['issues'].append({
                'type': 'DMARC Issue',
                'description': dmarc_status,
                'severity': dmarc_severity
            })
        ...
        if dkim_severity != 'Low':
            result['issues'].append({
                'type': 'DKIM Issue',
                'description': dkim_status,
                'severity': dkim_severity
            })
        
        # Determine overall risk level
        severity_levels = [get_severity_level(issue['severity']) for issue in result['issues']]
        if severity_levels:
        ...
        logging.error(f"Error analyzing email security: {e}")
        return {
            'domain': domain,
            'error': str(e),
            'risk_level': 'Unknown'
    ...
    try:
        ...
        security_headers = {
            ...
            'Permissions-Policy': {
                'present': False, 
                'value': None, 
                'description': 'Controls which browser features can be used (newer version of Feature-Policy)'
            },
            'Access-Control-Allow-Origin': {
                'present': False, 
                'value': None, 
                'description': 'Indicates which origins can access the resource'
            },
            'Cache-Control': {
                'present': False, 
                'value': None, 
            ...
            'Clear-Site-Data': {
                'present': False, 
                'value': None, 
                'description': 'Clears browsing data associated with the site'
            }
        }
            ...
            if header.lower() in [h.lower() for h in response.headers]:
                ...
                for h in response.headers:
                    if h.lower() == header.lower():
                        security_headers[header]['value'] = response.headers[h]
                        break
...
def detect_cms(url):
    ...
    try:
            ...
            for cms, details in cms_signatures.items():
                for path in details['common_paths']:
                    try:
                        ...
                        path_response = requests.head(check_url, headers=headers, timeout=5, verify=True)
        ...
        if detected_cms and detected_version:
            ...
            elif detected_cms == 'Joomla' and detected_version:
                # Add Joomla version checks
                version_parts = [int(x) for x in detected_version.split('.')]
                if version_parts[0] < 4:
                    result['potential_vulnerabilities'].append({
                        'name': 'Outdated Joomla',
                        'description': 'This Joomla version may contain known security vulnerabilities',
                        'recommendation': 'Update to the latest version of Joomla'
                    })
...
def crawl_for_sensitive_content(url, max_urls=10):
    ...
    try:
        ...
        disallowed_paths = []
        try:
            robots_response = requests.get(robots_url, headers=headers, timeout=5, verify=True)
        ...
        for path in sensitive_paths[:max_urls]:  # Limit to max_urls
            try:
                check_url = f"https://{base_url}{path}"
                path_response = requests.head(check_url, headers=headers, timeout=5, verify=True, allow_redirects=False)
        ...
        for file_ext in exposed_file_types:
            ...
            for path in check_paths:
                try:
                    check_url = f"https://{base_url}{path}"
                    file_response = requests.head(check_url, headers=headers, timeout=3, verify=True)
