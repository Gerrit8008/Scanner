...
@app.route('/fix_db')
def fix_db():
    """Fix the database for Render deployment"""
    results = []
    try:
        # Get the actual database path from environment
        import os
        import uuid
        from datetime import datetime
        
        # Define database path - use environment variable or default
        CLIENT_DB_PATH = os.environ.get('CLIENT_DB_PATH', os.path.join(os.path.dirname(os.path.abspath(__file__)), 'client_scanner.db'))
        results.append(f"Using database: {CLIENT_DB_PATH}")
        ...
        cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='clients'")
        clients_table_exists = cursor.fetchone() is not None
        results.append(f"Clients table exists: {clients_table_exists}")
